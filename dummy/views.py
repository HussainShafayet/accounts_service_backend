...
from .serializers import SumSerializer
from dummy.models import CalculationCounter

class SumApiView(APIView):
    """
    API view to calculate the sum of two integers using ORM.
    It tracks how many times the sum API is called using the CalculationCounter model.
    """

    def _get_and_increment_count(self):
        # Get the counter row or create it if missing
        counter, created = CalculationCounter.objects.get_or_create(id=1, defaults={'count': 0})
        counter.count += 1
        counter.save()
